...
export const BottomNavBar = () => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full bg-surface-container-lowest/85 backdrop-blur-md flex justify-around items-center py-4 px-2 z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
      ...
      <Link
        href="/"
        className={`flex flex-col items-center gap-1 ${pathname === "/" || pathname === "/practice" ? "text-primary" : "text-slate-500"}`}
      >
        <span className="material-symbols-outlined" data-icon="mic_none" style={pathname === "/" || pathname === "/practice" ? { fontVariationSettings: "'FILL' 1" } : {}}>mic_none</span>
        <span className={`text-[10px] ${pathname === "/" || pathname === "/practice" ? "font-bold" : ""}`}>Practice</span>
      </Link>
      <Link
        href="/progress"
